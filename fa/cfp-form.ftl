-note-some-markdown = برخی از نشانه گذاری‌های مارک‌داون مجاز است.
-note-no-markdown = استفاده از مارک‌داون مجاز نیست.
-note-editable = قبل از انتشار می‌توانید این مورد را تغییر دهید.
-optional = اختیاری
f-title = فراخوان ارسال ارائه RustFest Global 2021
f-submission-deadline-t = مهلت ارسال
f-manage = اگر قبلاً پروپوزالی در قالب TOML دارید (از این صفحه یا یک کنفرانس سازگار دیگر خروجی گرفته اید) می توانید فایل را در اینجا وارد کنید:
-note-most-markdown = اکثر ساختارهای مارک‌داون مجاز هستند (تایپوگرافی، لیست ها، پیوندها و غیره)
f-form-notes = پر کردن تمام فیلدها الزامی است، به جز مواردی که به صراحت به عنوان "اختیاری" علامت گذاری شده اند.
f-submission-deadline =
    توجه داشته باشید، فراخوان ارسال ارائه RustFest Global **ددلاین ندارد**، فراخوان ارسال ارائه *به پایان نمی‌رسد*، اما برای اینکه ارائه شما برای یکی از رویدادهای آینده ما در نظر گرفته شود، معمولاً شما باید 3 الی 4 هفته قبل از تاریخ رویداد، ارائه خود را برای ما ارسال کنید.
    
    { "[" }این لینک را برای تاریخ های پایانی برای ارسال ارائه برای رویدادهای پیش‌رو ببینید](
    { $linkUpcomingEvents }
    )
f-manage-t = بارگذاری و مدیریت پروپوزال ها
f-manage-auth-btn = ارسال کد
f-manage-upload-btn = بارگذاری پروپوزال
f-manage-download-t = دریافت پروپوزال
f-proposal-audience-beginner-friendly-t = **مقدماتی** -- ارائه‌های فنی مبتدی پسند، مقداری تجربه برنامه نویسی توصیه می شود
f-proposal-audience-all-audiences-t = **همه افراد** -- بدون نیاز به تجربه برنامه نویسی، یک ارائه برای همه مخاطبان
f-submission-lang =
    زبانی را که قرار است فرم ارسال پروپوزال خود را با آن بنویسید انتخاب کنید.
    
    با تغییر زبان بالا، زبان دستورالعمل های این صفحه تغییر می کند.
    ما ارسال‌ها را به زبان‌های موجود می‌پذیریم و همه اعضای کمیته CFP که به آن نیاز دارند، ترجمه‌هایی را برای ارزیابی خود دریافت خواهند کرد.
    نوشتن پروپوزال ارسالی خود به هر یک از این زبان‌ها *به هیچ وجه* بر شانس شما برای انتخاب شدن برای صحبت *تأثیری نمی‌گذارد*، بنابراین زبانی را انتخاب که فکر می‌کنید با آن راحت‌تر هستید.
    
    { "[" }درباره پشتیبانی زبان RustFest Global بیشتر بدانید]({ $linkSupportedLanguages })
f-contact-email =
    ما تاییدیه پروپوزال دریافتی‌ را برای شما ارسال خواهیم کرد.
    تیم ها از ایمیل شما برای تماس با شما برای پیگیری و به روز رسانی در مورد فرآیند انتخاب استفاده می کنند.
    با احراز هویت با استفاده از این آدرس، می‌توانید وضعیت پروپوزال‌های خود را ویرایش، بررسی و پس بگیرید.
f-manage-download = …در اینجا می توانید پروپوزال خود را به صورت متنی دریافت کنید.
f-manage-auth = همچنین می توانید با آدرس ایمیل خود احراز هویت کنید تا موارد ارسالی موجود خود را مدیریت کنید:
f-manage-download-btn = دریافت پروپوزال
f-submission-t = ارسال
f-submission-lang-t = زبان ارسالی خود را انتخاب کنید
f-submission-presentation-lang-t = زبان ارائه
f-presentation-format-other = دیگر
f-submission-presentation-format-t = فرمت ارائه
f-submission-presentation-format-standard-t = { f-presentation-format-standard } (تا ۲۵ دقیقه)
f-submission-presentation-format-longform-t = { f-presentation-format-longform } (تا ۵۰ دقیقه)
f-proposal-t = ارائه
f-presentation-format-standard = ارائه استاندارد
f-proposal-title-t = عنوان
f-submission-presentation-format-other-t = { f-presentation-format-other } (در بخش «یادداشت» توضیح داده شود)
f-presentation-format-longform = ارائه طولانی
f-proposal-summary-t = خلاصه
f-proposal-description-t = توضیحات
f-proposal-audience-t = مخاطبین
f-proposal-audience-some-rust-t = ** پایه‌ای Rust ** - مقداری دانش پایه در مورد Rust یا برنامه نویسی سیستمی توصیه می شود
f-proposal-audience-advanced-rust-t = **پیشرفته Rust** - دانش کامل Rust و/یا برنامه نویسی سیستمی توصیه می شود
f-proposal-notes-t = یادداشت به برگزار کنندگان ({ -optional })
f-submitter-t = درباره شما
f-submitter-name-t = نام
f-submitter-tagline-t = شعار ({ -optional })
f-submitter-bio-t = بیوگرافی
f-proposal-highlight-t = رویداد پیشنهادی
event-none = همه
event-none-summary = این پروپوزال باید برای هر یک از رویدادهای آینده مناسب باشد.
f-submitter-bio =
    درباره خودتان بیشتر به ما و شرکت کنندگان بگویید. این متن در صفحات جلسه نمایش داده می شود.
    
    _{ -note-most-markdown }_  
    _{ -note-editable }_
f-submit-status-success-t = پروپوزال دریافت شد!
f-submit-status-success = از ارسال شما متشکریم، به زودی یک ایمیل تأیید دریافت خواهید کرد.
f-submit-btn = ثبت پروپوزال
f-submit-confirm-btn = تایید و ثبت
f-cancel-btn = لغو
errmsg-shorter = { $fieldname } باید کمتر از 240 کاراکتر باشد
f-contact-t = اطلاعات تماس
f-contact-email-t = آدرس ایمیل
f-submit-dialog-t = پروپوزال خود را ثبت کنید
f-submit-status-sending-verification = آدرس ایمیل خود را تایید کنید…
f-submit-status-enter-verification-code =
    ما یک کد عددی منحصر به فرد را به آدرس ایمیل شما ارسال کرده ایم:  
    __{ $email }__
    
    برای ثبت پروپوزال، کد زیر را وارد کنید:
f-submit-error-generic = هنگام تلاش برای ثبت پروپوزال خطایی روی داد. شاید بعدا دوباره امتحان کنید؟
f-proposal-audience =
    جلسه شما چه نوع مخاطبی را هدف قرار می دهد؟  
    چه کسی بیشترین بهره را از ارائه شما خواهد برد؟  
    لطفاً نوع مخاطبی را انتخاب کنید که برای پروپوزال شما مناسب‌تر خواهد بود:
f-proposal-notes =
    در اینجا می توانید یادداشت ها یا نکاتی را برای برگزارکنندگان و کمیته انتخاب بگذارید.
    
    _{ -note-most-markdown }_
f-submission-presentation-lang =
    اگر می‌خواهید به { $currentLang } ارائه دهید، این فیلد را تغییر ندهید.
    
    اگر می‌خواهید به زبان دیگری غیر از زبان پروپوزال ارائه دهید، حتی اگر در بالا ذکر نشده باشد، می‌توانید در اینجا به ما اطلاع دهید.
    در RustFest ما می‌خواهیم این امکان را برای همه فراهم کنیم که ایده‌های خود را ارائه دهند و کارهای خود را به بهترین شکل ممکن به نمایش بگذارند، و اطمینان حاصل کنیم که این ایده‌ها به مخاطبان گسترده‌ای می‌رسند.
    به همین دلیل ما خوشحال می‌شویم که ارائه‌ها را به زبان های مختلف میزبانی کنیم، مشروط بر اینکه بتوانیم به اندازه کافی از آنها پشتیبانی کنیم (مثلا به صورت زیرنویس).
    
    به راحتی می توانید چندین زبان را در اینجا پیشنهاد کنید (با توجه به ترتیب اولویت خود)، و در صورت پذیرش، برای بحث در مورد جزئیات تماس خواهیم گرفت.
    
    { "[" }درباره پشتیبانی زبان RustFest Global بیشتر بدانید]({ $linkSupportedLanguages })
f-proposal-summary =
    شرح کوتاه یا خلاصه ای از جلسه شما.
    شما حداکثر 240 کاراکتر برای ارائه جلسه پیشنهادی خود، هم برای کمیته CFP(ارائه) و هم برای مخاطبان کنفرانس دارید.
    
    { f-proposal-title-t } و { f-proposal-summary-t } اولین چیزهایی هستند که توسط کمیته انتخاب برنامه مشاهده می شود، و همچنین در لیست های برنامه رویداد و شبکه‌های اجتماعی نمایش داده می شوند. اینها باارزش‌ترین مکان‌هایی هستند که می‌توانید تأثیر خوبی برای انتخاب ارائه و در صورت پذیرش پروپزال شما برای شرکت‌کنندگان در کنفرانس به جای بگذارید.
    
    _{ -note-some-markdown }_  
    _{ -note-editable }_
f-submission-presentation-format =
    ما در درجه اول به دنبال ارائه هستیم، اما اگر ایده ای برای مشارکت بدون ارائه دارید، در صورت تمایل به اشتراک بگذارید و اگر ما را متقاعد کردید، برای گفتگو در مورد جزئیات با شما تماس خواهیم گرفت.
    
    ما ارائه‌های استاندارد (حداکثر 25 دقیقه) و همچنین ارائه‌های عمیق تر "طولانی" را برای موضوعاتی می پذیریم که به زمان بیشتری برای توضیح دادن نیاز دارند - در مورد دوم، مطمئن شوید که دلیل نیاز به زمان بیشتر را در پروپزال یا «یادداشت ‌ها» مینویسید.
f-proposal-description =
    شرح طولانی تر و دقیق تر از جلسه پیشنهادی شما.
    
    برای اطلاعات بیشتر در مورد نحوه ساختار پروپوزال خود، [دستورالعمل های CFP] منتشر شده ما را بخوانید ({ $linkCfpGuidelines }).
    
    _{ -note-most-markdown }_  
    _{ -note-editable }_
